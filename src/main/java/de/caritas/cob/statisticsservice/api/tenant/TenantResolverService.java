...
public class TenantResolverService {
  ...
  private Long resolveForNonAuthenticatedUser(HttpServletRequest request) {
    ...
    if (multitenancyWithSingleDomain) {
      // do not throw AccessDenied to enable access to public endpoints like swagger
      log.warn("Resolved to null tenant ");
      return null;
    } else {
      return resolveForNonAuthenticatedUserInNormalMultitenancyMode(request);
    }
  }

  private Long resolveForNonAuthenticatedUserInNormalMultitenancyMode(HttpServletRequest request) {
